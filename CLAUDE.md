...
事件全部经 `Game.handle_event` 分发。**选择（多选组 `Game.selected` 列表，主选 `selected_mech`=第一个）**：左键点机兵=单选、Shift+点=加/减选、**空地拖拽=框选**；用拖拽起点消歧——起点在机兵上=画移动路径，起点在空地=框选。**指令**：从选中成员拖拽=（单选）徒手路径/（多选）编队移动到终点（`_move_group` 按 `_formation` 分配集结点）；空地单击=移动到点；右键点敌人=整组指定攻击、右键空地=整组停止。**编组**：`Ctrl+1~6` 设编组、`1~6`（仅 WAVE）选组，否则 `1~5` 回落为建造快捷键（见 `_number_key`）。其余：`空格`=暂停、`N`=下一波、`Q/E`=技能、`Z/X/C`=全局技能、`M`=静音、`滚轮/Tab`=旋转挡墙、`ESC`=取消选择/建造/技能瞄准、`R`=重开。HUD 技能槽与左上全局技能栏可点击释放。
